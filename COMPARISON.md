...
| **Local providers** | Ollama, LM Studio, llama.cpp | Ollama (via OpenAI-compat) | Ollama | n/a |
...
| **Multimodal (audio)** | ✅ Google, OpenAI, llama.cpp | ✅ partial | ❌ | ✅ |
| **Multimodal (PDF)** | ✅ Google, OpenAI, Anthropic | ✅ | ❌ | ✅ |
| **Unsupported file handling** | ✅ Raises `UnsupportedFileError` | Provider error | Provider error | Provider error |
...
| **Proxy / gateway mode** | ❌ | ✅ | ❌ | ❌ |
| **Stateless by design** | ✅ | ✅ | ✅ | ✅ |
| **Remote file cleanup** | ✅ (Google uploads) | ✅ auto | n/a | n/a |

## Where UnifiedAiClient Stands Out

**Zero heavy dependencies.** Every provider except Google is implemented via `urllib`
from the standard library, with no SDK juggling and no transitive dependency bloat. The entire
